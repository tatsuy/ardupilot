...
MAV_MODE GCS_MAVLINK_Copter::base_mode() const
{
    ...
    switch (copter.control_mode->mode_number()) {
...
{
    return (uint32_t)copter.control_mode->mode_number();
}
    ...
    case MAV_CMD_SOLO_BTN_PAUSE_CLICK: {
        ...
        if (copter.motors->armed()) {
            if (copter.ap.land_complete) {
                ...
                bool shot_mode = (!is_zero(packet.param1) && (copter.control_mode == &copter.mode_guided || copter.control_mode == &copter.mode_guided_nogps));
