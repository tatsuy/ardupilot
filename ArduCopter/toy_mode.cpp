...
void ToyMode::update()
{
...
#if 0
    copter.ahrs.set_indoor_mode(copter.flightmode == &copter.mode_althold || copter.flightmode == &copter.mode_flowhold);
#endif
    ...
    if ((flags & FLAG_THR_ARM) && throttle_near_max && !copter.motors->armed()) {
        ...
        if (throttle_high_counter >= TOY_LAND_ARM_COUNT) {
            ...
            arm_check_compass();
            if (!copter.arming.arm(AP_Arming::Method::MAVLINK) && (flags & FLAG_UPGRADE_LOITER) && copter.flightmode == &copter.mode_loiter) {
                /*
    ...
    if (upgrade_to_loiter) {
        if (!copter.motors->armed() || copter.flightmode != &copter.mode_althold) {
    ...
    if (copter.flightmode == &copter.mode_rtl && (flags & FLAG_RTL_CANCEL) && throttle_near_max) {
    ...
    if (!copter.motors->armed() && (copter.flightmode == &copter.mode_land || copter.flightmode == &copter.mode_rtl)) {
