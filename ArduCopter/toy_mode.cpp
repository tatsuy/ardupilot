...
void ToyMode::update()
{
...
#if 0
    copter.ahrs.set_indoor_mode(copter.control_mode == &copter.mode_althold || copter.control_mode == &copter.mode_flowhold);
#endif
    ...
    if ((flags & FLAG_THR_ARM) && throttle_near_max && !copter.motors->armed()) {
        ...
        if (throttle_high_counter >= TOY_LAND_ARM_COUNT) {
            ...
            arm_check_compass();
            if (!copter.arming.arm(AP_Arming::Method::MAVLINK) && (flags & FLAG_UPGRADE_LOITER) && copter.control_mode == &copter.mode_loiter) {
                /*
    ...
    if (upgrade_to_loiter) {
        if (!copter.motors->armed() || copter.control_mode != &copter.mode_althold) {
    ...
    if (copter.control_mode == &copter.mode_rtl && (flags & FLAG_RTL_CANCEL) && throttle_near_max) {
        gcs().send_text(MAV_SEVERITY_INFO, "Tmode: RTL cancel");        
        set_and_remember_mode(Mode::Number::LOITER, ModeReason::TOY_MODE);
    }
    
    enum Mode::Number old_mode = copter.control_mode->mode_number();
    ...
    if (!copter.motors->armed() && (copter.control_mode == &copter.mode_land || copter.control_mode == &copter.mode_rtl)) {
        // revert back to last primary flight mode if disarmed after landing
        new_mode = Mode::Number(primary_mode[last_mode_choice].get());
    }
    
    if (new_mode != copter.control_mode->mode_number()) {
        load_test.running = false;
#if AC_FENCE == ENABLED
        copter.fence.enable(false);
#endif
